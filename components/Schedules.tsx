...
export const Schedules: React.FC<SchedulesProps> = ({ schedules, setSchedules, members, songs }) => {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      ...
      {isAdding && (
        <form onSubmit={addSchedule} className="bg-white p-8 rounded-[2.5rem] border border-indigo-100 shadow-2xl space-y-8 animate-in zoom-in-95 duration-200 max-w-5xl mx-auto overflow-hidden">
          ...
          <div className="flex justify-end gap-4 pt-6 border-t border-slate-50">
            ...
            <button type="submit" className="px-12 py-3.5 bg-indigo-600 text-white font-black uppercase text-xs tracking-[0.2em] rounded-2xl hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all">Publicar Escala</button>
          </div>
        </form>
      )}
      ...
      <div className="space-y-6">
        {schedules.map((sch) => (
          <div key={sch.id} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 flex flex-col md:flex-row gap-8 hover:shadow-xl transition-all group relative overflow-hidden">
            ...
            <div className="md:w-52 flex-shrink-0 flex flex-col items-center justify-center p-8 bg-indigo-50 rounded-3xl border border-indigo-100 text-indigo-700">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {sch.members.map(mId => {
                      ...
                      return member ? (
                        <div key={mId} className="bg-slate-100 text-slate-700 px-4 py-1.5 rounded-xl text-xs font-bold shadow-sm">
                          {member.name}
                        </div>
                      ) : null;
                    })}
        ...
        {schedules.length === 0 && (
          <div className="text-center py-20 bg-white rounded-[2.5rem] border border-dashed border-slate-200 text-slate-400">
            <CalendarDays size={64} className="mx-auto mb-4 opacity-10" />
